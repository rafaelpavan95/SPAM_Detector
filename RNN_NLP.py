...
class NLP(nn.Module):
  def __init__(self, n_vocab, embed_dim, n_hidden, n_rnnlayers, n_outputs, device):
    ...
    self.V = n_vocab # number of words in the vocab
    self.D = embed_dim # size of the vector space in which words will be embedded
    self.M = n_hidden # size hidden layers
    self.K = n_outputs 
    self.L = n_rnnlayers # number of stacked layers of the LSTM
    self.device = device
  ...
  for it in range(epochs):
    ...
    if it>1:
      ...
      if np.min(np.asarray(test_losses))<test_losses[it]:
        ...
        torch.save(model.state_dict(), 'NLP_RNN.pt')
